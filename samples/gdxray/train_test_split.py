...
train_root_dir = os.path.normpath(args.dataset) + '_train/'
val_root_dir = os.path.normpath(args.dataset) + '_val/'
...
for source_path in train_list:
    dest_path = source_path.replace(args.dataset, train_root_dir)
...
for source_path in val_list:
    dest_path = source_path.replace(args.dataset, val_root_dir)
    # print('Source:{}, Dest:{}'.format(source_path, dest_path))
    os.makedirs(os.path.dirname(dest_path), exist_ok=True)
    copyfile(source_path, dest_path)
